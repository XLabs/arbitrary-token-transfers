...
pub struct ForeignContractAccount {
...
impl ForeignContractAccount {
    ...
    /// this account.
    pub fn verify<M>(&self, vaa: &token_bridge::PostedTransferWith<M>) -> bool
    where
        M: Copy + Clone + AnchorSerialize + AnchorDeserialize,
    {
...
pub mod test {
    ...
    fn test_checked_token_fee() -> Result<()> {
        ...
        let mut relayer_fee = ForeignContractAccount {
