...
import { TokenBridgeRelayer } from '../../../../target/idl/token-bridge-relayer';
import IDL from '../../../../target/idl/token-bridge-relayer.json';
import {
...
type TbrConfigAccount = anchor.IdlAccounts<TokenBridgeRelayer>['tbrConfigAccount'];
type ChainConfigAccount = anchor.IdlAccounts<TokenBridgeRelayer>['chainConfigAccount'];
type PeerAccount = anchor.IdlAccounts<TokenBridgeRelayer>['peerAccount'];
type ForeignContractAccount = anchor.IdlAccounts<TokenBridgeRelayer>['foreignContractAccount'];
type SignerSequenceAccount = anchor.IdlAccounts<TokenBridgeRelayer>['signerSequenceAccount'];

export interface TbrAddresses {
  config(): PublicKey,
  chainConfig(chain: Chain): PublicKey,
  peer(chain: Chain, peerAddress: UniversalAddress): PublicKey,
  foreignContract(chain: Chain): PublicKey,
...
export interface ReadTbrAccounts {
  ...
  foreignContract(chain: Chain): Promise<ForeignContractAccount>,
...
export class TbrClient {
  ...
  get address(): TbrAddresses {
    return {
      ...
      foreignContract: (chain: Chain) => pda.foreignContract(this.program.programId, chain),
  ...
  get read(): ReadTbrAccounts {
    return {
      config: () => this.program.account.tbrConfigAccount.fetch(this.address.config()),
      chainConfig: (chain: Chain) => this.program.account.chainConfigAccount.fetch(this.address.chainConfig(chain)),
      peer: (chain: Chain, peerAddress: UniversalAddress) => this.program.account.peerAccount.fetch(this.address.peer(chain, peerAddress)),
      foreignContract: (chain: Chain) => this.program.account.foreignContractAccount.fetch(this.address.foreignContract(chain)),
      signerSequence: (signer: PublicKey) => this.program.account.signerSequenceAccount.fetch(this.address.signerSequence(signer)),
    };
  }

  async initialize(owner: PublicKey, quoterProgram: PublicKey): Promise<web3.TransactionInstruction> {
    return this.program.methods.initialize(quoterProgram).accounts({
      owner,
    }).instruction()
  }
  ...
  async registerPeer(signer: PublicKey, chain: Chain, peerAddress: UniversalAddress): Promise<web3.TransactionInstruction> {
    return this.program.methods.registerPeer(chainToChainId(chain), Array.from(peerAddress)).accountsStrict({
      ...
      peerAccount: this.address.peer(chain, peerAddress),
      chainConfig: this.address.chainConfig(chain),
      systemProgram: SYSTEM_PROGRAM_ID,
    }).instruction()
  }

  async updateCanonical(signer: PublicKey, chain: Chain, peerAddress: UniversalAddress): Promise<web3.TransactionInstruction> {
    return this.program.methods.updateCanonical(chainToChainId(chain), Array.from(peerAddress)).accountsStrict({
      owner: signer,
      tbrConfig: this.address.config(),
      peerAccount: this.address.peer(chain, peerAddress),
      chainConfig: this.address.chainConfig(chain),
  ...
  async updateQuoterProgramAddress(signer: PublicKey, quoterProgramAddress: PublicKey): Promise<web3.TransactionInstruction> {
    return this.program.methods.updateQuoterProgramAddress(quoterProgramAddress).accounts({
      signer,
    }).instruction()
  }
  ...
  async transferNativeTokens(signer: PublicKey, params: TransferNativeParameters): Promise<web3.TransactionInstruction> {
    ...
    return this.program.methods.transferNativeTokens(
      ...
      foreignContract: this.address.foreignContract(recipientChain),
      mint,
      userTokenAccount,
...
const pda = {
  ...
  foreignContract: (programId: PublicKey, chain: Chain): PublicKey => {
    return PublicKey.findProgramAddressSync(
      [
