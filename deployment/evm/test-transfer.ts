...
import { inspect } from "util";
  ...
  async function run() {
    ...
    for (const chain of chains.filter((chain) => chain.chainId === 10002)) {
        ...
        const transfers = await Promise.all(chains.filter((targetChain) => chain.chainId !== targetChain.chainId).map(async (chain) => {

          const address = await signer.getAddress();

          const feeEstimation = (await tbrv3.relayingFee({
          ...
          return {
            args: {
              ...
              inputAmount: 1n,
              gasDropoff: 0n,
              recipient: { chain: chain.name as Chain, address: toUniversal(chain.name as Chain , address)},
